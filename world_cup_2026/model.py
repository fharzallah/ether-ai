...
class DixonColesModel:
    ...
    def fit(self, games):
        ...
        for _, row in games.iterrows():
            ...
            weight = np.exp(-self.phi * (days_ago / 365.0))
            games_data.append((team_to_idx[row['Home']], team_to_idx[row['Away']], row['HomeGoals'], row['AwayGoals'], weight))
        init_params = np.concatenate([np.zeros(n_teams), np.zeros(n_teams), [0.1], [0.0]])
        res = minimize(log_likelihood, init_params, args=(games_data, n_teams, self.phi), method='L-BFGS-B', options={'maxiter': 50})
...
if __name__ == "__main__":
    ...
    model = DixonColesModel(phi=0.5)
    model.fit(df_matches_wc)
    results = {'alpha': model.alpha, 'beta': model.beta, 'gamma': model.gamma, 'rho': model.rho}
